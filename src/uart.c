...
void UART_SetBaudRate(uint32_t v_baudRate_u32)
{
  ...
  USART0_BAUDL = util_ExtractByte0to8(RegValue);
  USART0_BAUDH = util_ExtractByte8to16(RegValue);
}
...
char UART_RxChar(void)
{
  ...
  return(UDR);*/                          // return the received char
	while(util_IsBitCleared(USART0_STATUS,USART_RXCIF_bp));
...
void UART_TxChar(char v_uartData_u8)
{
  ...
  while(util_IsBitCleared(USART0_STATUS,USART_DREIF_bp));
  USART0_TXDATAL = v_uartData_u8;
}
